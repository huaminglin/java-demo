...
public class FloatSignificandPrecisionDemo {
    ...
    private static void printFloatIntPrecision(int start) {
        ...
        for (int i = start; i < Integer.MAX_VALUE; i++) {
            ...
            int fi = (int) f;
            if (fi != i) {
                System.out.println(i + "(" + Integer.toHexString(i) + ") -> " + fi + "(" + Long.toHexString(fi) + ")");
                found += 1;
                if (found >= 5) {
                    break;
                }
    ...
    private static void printDoubleLongPrecision(long start) {
        ...
        for (long i = start; i < Long.MAX_VALUE; i++) {
            ...
            long fi = (long) f;
            if (fi != i) {
                System.out.println(i + "(" + Long.toHexString(i) + ") -> " + fi + "(" + Long.toHexString(fi) + ")");
                found += 1;
                if (found >= 5) {
                    break;
                }
    ...
    public static void main(String[] args) {
        ...
        printDoubleLongPrecision((long) Math.pow(2, 54)); // Start with 0 and it will take a long time. 2^53 is a big number.
    }
}
