...
public class RestWSProvider implements Provider<Source> {
  ...
  private String makeMsg(MessageContext messageContext) {
    ...
    sb.append(", User agent: " + ((Map) messageContext.get(MessageContext.HTTP_REQUEST_HEADERS))
        .get("user-agent"));
    sb.append(", Content type: " + ((Map) messageContext.get(MessageContext.HTTP_REQUEST_HEADERS))
        .get("content-type"));
    sb.append(", Http accept: " + ((Map) messageContext.get(MessageContext.HTTP_REQUEST_HEADERS))
        .get("accept"));
    sb.append(", Http host: " + ((Map) messageContext.get(MessageContext.HTTP_REQUEST_HEADERS))
        .get("host"));
    sb.append(", Connection: " + ((Map) messageContext.get(MessageContext.HTTP_REQUEST_HEADERS))
        .get("connection"));
  ...
  public Source invoke(Source request) {
    ...
    String content =
        "<response><content>" + makeMsg(messageContext) + "</content><request><![CDATA["
            + strRequest + "]]]></request></response>";
