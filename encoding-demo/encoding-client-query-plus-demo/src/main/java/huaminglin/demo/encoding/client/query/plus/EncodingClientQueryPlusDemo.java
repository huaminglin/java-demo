...
//            at org.springframework.web.util.HierarchicalUriComponents.verifyUriComponent(HierarchicalUriComponents.java:415)
    }
    {
      ...
      if (UriComponentsBuilder.fromHttpUrl(preEncodedUrl).build(encoded).toUriString()
          .equals(preEncodedUrl)) {
        System.out
            .println("UriComponentsBuilder verifies that the url is already encoded correctly.");
  ...
  private static void buildWithTemplateEncodingAndImplicitExpand() {
    ...
    System.out.println(
        "Note: the \"+\" in the URI variable is encoded, but '+' outside URI variable is not encoded.");
  }
